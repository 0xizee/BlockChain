...
const WS = require("ws");
const { AyushChain } = require("./MyCoin");
...
setTimeout(() => {
	if (AyushChain.transactions.length !== 0) {
		AyushChain.mineTransactions(publicKey);

		sendMessage(produceMessage("TYPE_REPLACE_CHAIN", [
			AyushChain.getLastBlock(),
			AyushChain.difficulty
		]))
	}
}, 6500);
